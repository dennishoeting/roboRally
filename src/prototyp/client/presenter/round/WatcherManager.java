 ...
 */
public class WatcherManager implements ManagerInterface {
	...
	public class AnimationTimer extends AnimationTimerAbstract {
		...
		private class LaserAnimationListener extends AnimationListener {
			...
			public boolean animationReady() {
				if (++this.n == this.animateList.size()) {
					...
					for (Robot hit : this.hittenRobots) {
						if (hit.getDamageToken() >= 10) {
							...
							WatcherManager.this.presenter.getOthersStateRecords().get(hit.getPlayer().getUser().getId())
									.updateDamageToken(hit.getDamageToken());
							WatcherManager.this.presenter.getOthersStateRecords().get(hit.getPlayer().getUser().getId())
									.setNumberOfLifeToken(hit.getLifePoints() + 1);
							WatcherManager.this.presenter.getPage().getRobotStatusArea().getOthersReadyGrid().redraw();
							WatcherManager.this.presenter.getPage().getRobotStatusArea().getOthersStateGrid().redraw();
							...
							if (hit.getLifePoints() == 0) {
								...
								InformationWindow.showWindow(
										WatcherManager.this.presenter.getPage(),
								...
								WatcherManager.this.presenter.getOthersStateRecords().get(hit.getPlayer().getUser().getId())
										.setDead();
								WatcherManager.this.presenter.getOthersReadyRecords().get(hit.getPlayer().getUser().getId())
										.setDead();
								WatcherManager.this.presenter.getPage().getRobotStatusArea().getOthersReadyGrid().redraw();
								WatcherManager.this.presenter.getPage().getRobotStatusArea().getOthersStateGrid().redraw();
							}
							...
							hit.setI(-1);
							hit.setJ(-1);
							WatcherManager.this.presenter.getPage().getDrawingArea()
									.remove(WatcherManager.this.presenter.getRobotsImageList().get(hit));
						}
		...
		private void addCompactors(final List<List<Animate>> animateList, final Timer timer) {
			...
			subList.add(new Animate(new Line(0, 0, 0, 0), "", 0, 0, 1000) {
				public void onStart() {
					 ...
					 * Aktive Bilder
					 */
					for (CompactorField beltField : WatcherManager.this.playingBoard.getCompactorFieldList()) {
						if (!beltField.isActiveInFirstAndFifth() || (AnimationTimer.this.i == 0 || AnimationTimer.this.i == 4)) {
							final Image img = WatcherManager.this.presenter.getFieldsImageList()[beltField.getI()][beltField
									.getJ()].get(beltField.getImagePathOff());
			 ...
			 */
			for (final Robot robot : WatcherManager.this.robots.values()) {
				if (!(robot.isDead() || robot.isDeadForTurn() || robot.getI() == -1)) {
					 ...
					 */
					if (WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()] instanceof CompactorField
							&& (((CompactorField) WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()])
		...
		private void addConveyorBelts(final List<List<Animate>> animateList, final boolean isFirstTurn, final Timer timer) {
			...
			final List<ConveyorBeltField> conveyorBeltFieldList = new ArrayList<ConveyorBeltField>(
					WatcherManager.this.playingBoard.getConveyorBeltFieldRangeTwoList());
			...
			if (!isFirstTurn) {
				 ...
				 */
				conveyorBeltFieldList.addAll(WatcherManager.this.playingBoard.getConveyorBeltFieldRangeOneList());
			}
			...
			for (final Robot robot : robotsOnConveyor.keySet()) {
				...
				if (WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()] instanceof ConveyorBeltField) {
					outDirection = ((ConveyorBeltField) WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()])
							.getArrowOutDirection();
				} else {
					outDirection = ((CompactorField) WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()])
							.getDirection();
				}
			...
			conveyorList.add(new Animate(new Line(0, 0, 0, 0), "", 0, 0, 1500) {
				...
				public void onStart() {
					...
					for (ConveyorBeltField beltField : WatcherManager.this.playingBoard.getConveyorBeltFieldRangeTwoList()) {
						final Image img = WatcherManager.this.presenter.getFieldsImageList()[beltField.getI()][beltField.getJ()]
								.get(beltField.getImagePathOff());
						img.setHref(beltField.getImagePathOn());
					}
					if (!isFirstTurn) {
						for (ConveyorBeltField beltField : WatcherManager.this.playingBoard.getConveyorBeltFieldRangeOneList()) {
							final Image img = WatcherManager.this.presenter.getFieldsImageList()[beltField.getI()][beltField.getJ()]
									.get(beltField.getImagePathOff());
							img.setHref(beltField.getImagePathOn());
						}
			...
			for (final Robot robot : robotsOnConveyor.keySet()) {
				...
				if (robot.getI() != oldPos.getI()) {
					conveyorList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robot), "y",
					...
					conveyorList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robot), "x",
			...
			for (final Robot robot : robotsOnConveyor.keySet()) {
				if (robotsMoveIntoDirection.get(robot) != null && WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()] instanceof ConveyorBeltField
						&& ((ConveyorBeltField) WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()])
								.isMerging()) {
					...
					if ((robotMovement = Direction.getRobotMovement(robotsMoveIntoDirection.get(robot),
						...
						turnList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robot), "rotation", 0,
								robotMovement == RobotMovement.TURN_RIGHT ? 90 : -90, 1000) {

							public void onComplete() {
								final Image curr = new Image(robot.getJ() * 50, robot.getI() * 50, 50, 50,
										robot.getImagePath());
								final Image old = WatcherManager.this.presenter.getRobotsImageList().get(robot);
								WatcherManager.this.presenter.getPage().getDrawingArea().add(curr);
								old.setVisible(false);
								old.setRotation(0);
								old.setHref(robot.getImagePath());
								old.setVisible(true);					
								WatcherManager.this.presenter.getPage().getDrawingArea().remove(curr);
			...
			for (final Robot fallen : WatcherManager.this.robots.values()) {
				if (fallen.getI() != -1
						&& WatcherManager.this.playingBoard.getFields()[fallen.getI()][fallen.getJ()] instanceof HoleField) {
		...
		private void addGears(final List<List<Animate>> animateList, final Timer timer) {
			 ...
			 */
			if (WatcherManager.this.playingBoard.getGearFieldList().size() > 0) {
				...
				for (GearField gearField : WatcherManager.this.playingBoard.getGearFieldList()) {
					...
					if (gearField.isClockwiseDirection()) {
						...
						if ((robot = getRobotByPosition(gearField.getI(), gearField.getJ())) != null) {
							...
							gearFieldList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robot),
									"rotation", 0, 90, 1000) {
								...
								public void onComplete() {
									...
									WatcherManager.this.presenter.getRobotsImageList().get(robot).setVisible(false);
									WatcherManager.this.presenter.getPage().getDrawingArea()
											.remove(WatcherManager.this.presenter.getRobotsImageList().get(robot));
									WatcherManager.this.presenter.getRobotsImageList().put(robot, curr);
						 ...
						 */
						// Großes Zahnrad
						Image img = WatcherManager.this.presenter.getFieldsImageList()[gearField.getI()][gearField.getJ()]
						...
						// kleines Zahnrad
						img = WatcherManager.this.presenter.getFieldsImageList()[gearField.getI()][gearField.getJ()]
		...
		private void addPusher(final List<List<Animate>> animateList, final Timer timer) {
			 ...
			 */
			if (WatcherManager.this.playingBoard.getPusherFieldList().size() > 0) {
				...
				for (PusherField pusherField : WatcherManager.this.playingBoard.getPusherFieldList()) {
					...
					if (!pusherField.isActiveInUneven() || this.i % 2 == 0) {
						...
						case NORTH:
							 ...
							 */
							img = WatcherManager.this.presenter.getFieldsImageList()[pusherField.getI()][pusherField.getJ()]
		...
		private List<Animate> addRobotLaser(final Timer timer) {
			 ...
			 * Roboterlaser
			 */
			if (WatcherManager.this.roundOption.isRobotShootsOn()) {
				...
				robotLaserShot: for (final Robot robot : WatcherManager.this.robots.values()) {

					if (robot.getI() != -1 && robot.getPowerDown() != 2) {
						Field hangler = WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()];
						...
						while (hangler != null) {
							...
							if ((hit = getRobotByPosition(hangler.getI(), hangler.getJ())) != null) {
								...
								WatcherManager.this.presenter.getPage().getDrawingArea().add(laser);
								...
								robotLaserAnimation.add(new Animate(laser, axis, start, robotHit + end + range * 50, (Math
									...
									public void onComplete() {
										...
										WatcherManager.this.presenter.getOthersStateRecords()
												.get(hit.getPlayer().getUser().getId()).updateDamageToken(hit.getDamageToken());
										WatcherManager.this.presenter.getPage().getRobotStatusArea().getOthersStateArea()
												.redraw();
										...
										new Timer() {
											@Override
											public void run() {
												WatcherManager.this.presenter.getPage().getDrawingArea().remove(laser);
		...
		private void addWallLaser(final List<List<Animate>> animateList, final Timer timer) {
			...
			final LaserAnimationListener laserAnimationListener = new LaserAnimationListener(timer, wallLaserList, 100,
					hittenRobots);
			for (final LaserCannonField laserCannonField : WatcherManager.this.playingBoard.getLaserCannonFieldList()) {
				...
				if (laserCannonField.getLaserCannonInfo().getCannonsNorth() > 0) {
					...
					case 1: // Anzahl der Laser 1
						...
						WatcherManager.this.presenter.getPage().getDrawingArea().add(laser1);
							...
							public void onComplete() {
								if (hit != null) {
									...
									new Timer() {
										@Override
										public void run() {
											WatcherManager.this.presenter.getPage().getDrawingArea().remove(laser1);
											laserAnimationListener.animationReady();
										}
					...
					case 2: // Anzahl der Laser 2
						...
						WatcherManager.this.presenter.getPage().getDrawingArea().add(laser2);
							...
							public void onComplete() {
								if (hit != null) {
									...
									new Timer() {
										@Override
										public void run() {
											WatcherManager.this.presenter.getPage().getDrawingArea().remove(laser2);
											laserAnimationListener.animationReady();
										}
					...
					case 3: // Anzahl der Laser 3
						...
						WatcherManager.this.presenter.getPage().getDrawingArea().add(laser3);
							...
							public void onComplete() {
								if (hit != null) {
									...
									new Timer() {
										@Override
										public void run() {
											WatcherManager.this.presenter.getPage().getDrawingArea().remove(laser3);
											laserAnimationListener.animationReady();
										}
		...
		private List<Animate> createFallAnimation(final Timer timer, final Robot fallen,
			 ...
			 */
			fallenRobotList
					.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(fallen), "width", 50, 0, 1000) {
						public void onComplete() {
							fallAnimationListener.animationReady();
						}
					});
			fallenRobotList
					.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(fallen), "height", 50, 0, 1000) {
						public void onComplete() {
							fallAnimationListener.animationReady();
						}
					});

			fallenRobotList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(fallen), "rotation", 0, 360,
					1000) {
				public void onComplete() {
					fallAnimationListener.animationReady();
				}
			...
			fallenRobotList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(fallen), "x",
			...
			fallenRobotList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(fallen), "y",
					fallen.getI() * 50, fallen.getI() * 50 + 25, 1000) {

				public void onComplete() {

					WatcherManager.this.presenter.getPage().getDrawingArea()
							.remove(WatcherManager.this.presenter.getRobotsImageList().get(fallen));
		...
		private List<List<Animate>> move(final Robot robot, final Direction direction, final Timer timer, final boolean[] moved,
			...
			if (!robot.isDeadForTurn()) {
				 ...
				 */
				Field hanglerField = WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()];
				...
				while (hanglerField != null) {
					...
					if ((r = getRobotByPosition(hanglerField.getI(), hanglerField.getJ())) == null || r.isDeadForTurn()) {
						...
						for (int n = 0; n < wayList.size(); n++) {
							...
							moveList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robotList.get(n)),
				...
				if (WatcherManager.this.playingBoard.getFields()[fallenRobot.getI()][fallenRobot.getJ()] instanceof HoleField) {
				...
				if (animateList.size() == 0) {
					 ...
					 */
					forwardList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robotList.get(0)),
							movement, movement.equals("y") ? robotList.get(0).getI() * 50 : robotList.get(0).getJ() * 50,
							movement.equals("y") ? robotList.get(0).getI() * 50 + stepWall : robotList.get(0).getJ() * 50
									+ stepWall, 200) {
					...
					for (int i = 1; i < robotList.size() - 1; i++) {
						forwardList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robotList.get(i)),
					...
					for (int i = 0; i < robotList.size(); i++) {
						backwardList.add(new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robotList.get(i)),
					...
					if (!robot.isHasPushedOnRepairfield()) {
						for (int i = 1; i < robotList.size(); i++) {
							if (WatcherManager.this.playingBoard.getFields()[robotList.get(i).getI()][robotList.get(i).getJ()] instanceof RepairField) {
		...
		private boolean pushRobots(final Robot robot, Map<Robot, Position> map, final Field.NeighbourDelegate del) {
			if (robot != null) {
				...
				final Field next = del.invoke(WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()]);
				if(next == null) {
					return false;
				}
		...
		public void run() {
			try {
				...
				if ((this.i = this.i + 1) >= 5) {
					...
					WatcherManager.this.animationTimer = null;

					if (WatcherManager.this.roundFinished == false) {
						/*
						 ...
						 * durchegührt haben
						 */
						for (Robot r : WatcherManager.this.robots.values()) {
							if (!r.isDead()) {
								...
								if (r.getPowerDown() == 1) {
									if (!r.isDeadForTurn()) {
										...
										WatcherManager.this.presenter.getOthersReadyRecords()
									...
									WatcherManager.this.presenter.getOthersStateRecords().get(r.getPlayer().getUser().getId())
											.updateDamageToken(0);
									WatcherManager.this.presenter.getPage().getRobotStatusArea().getOthersStateArea().redraw();
								}
							}
						}
					}
				...
				new Timer() {
					...
					public void run() {
						...
						if (++this.i == cardsList.size()) {
							...
							new Timer() {
								...
								private boolean allRobotsDead() {
									for (Robot robot : WatcherManager.this.robots.values()) {
										if (!robot.isDeadForTurn()) {
											return false;
										}
									}
								...
								private BackupField checkGameStatus() {
									...
									for (final Robot robot : WatcherManager.this.robots.values()) {
										...
										if (!robot.isDeadForTurn() && robot.getPowerDown() != 2) {
											 ...
											 * müsste
											 */
											if (WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()] == WatcherManager.this.playingBoard
												 ...
												 */
												if (robot.getNumberOfReachedCheckpoints() == WatcherManager.this.playingBoard
													...
													WatcherManager.this.presenter.getPage()
															.addChild(
																	new WinnerWindowPresenter(presenter.getRoundInfo()
																			.getRoundId(), WatcherManager.this.presenter, robot
																			.getPreviewImagePath(), robot.getPlayer().getUser()
																			.getAccountData().getNickname(), Page.props
																			.winnerwindow_information_other(), new ArrayList<Robot>(robots.values()), robot).getPage());

													WatcherManager.this.roundFinished = true;
											 ...
											 */
											if (WatcherManager.this.playingBoard.getFields()[robot.getI()][robot.getJ()] instanceof RepairField) {

												/*
												 * Sound abspielen
												 */
												RoundPlayerPagePresenter.SOUND_MANAGER.play("reparatur-"
														+ ((RepairField) WatcherManager.this.playingBoard.getFields()[robot
												...
												robot.setDamageToken(robot.getDamageToken()
														- ((RepairField) WatcherManager.this.playingBoard.getFields()[robot
																.getI()][robot.getJ()]).getNumberOfWrench());
												...
												robot.setPoints(robot.getPoints()
														+ ((RepairField) WatcherManager.this.playingBoard.getFields()[robot
																.getI()][robot.getJ()]).getNumberOfWrench() * 100);
						 ...
						 */
						final Robot robot = WatcherManager.this.robots.get(cardsList.get(this.i).getPlayerId());
						...
						if (!robot.isDeadForTurn() && robot.getPowerDown() != 2) {
							...
							case TURN_LEFT:
								...
								robot.setDirection(Direction.getDirection(robot.getDirection(), RobotMovement.TURN_LEFT));
								new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robot), "rotation", 0, -90,
										1000) {
							...
							case TURN_AROUND:
								...
								new Animate(WatcherManager.this.presenter.getRobotsImageList().get(robot), "rotation", 0, 180,
										1000) {
								...
								new Timer() {
									...
									public void run() {
										...
										if ((this.index += 1) == this.list.size()) {
											WatcherManager.this.animateReplayList.addAll(this.list);
		...
		private void setRobotDead(final Robot robot) {
			...
			WatcherManager.this.presenter.getOthersStateRecords().get(robot.getPlayer().getUser().getId())
					.setNumberOfLifeToken(robot.getLifePoints() + 1);

			// damagetoken wieder auf 0 setzten
			WatcherManager.this.presenter.getOthersStateRecords().get(robot.getPlayer().getUser().getId()).updateDamageToken(0);

			if (robot.getLifePoints() == 0) {
				robot.setDead(true);

				// Window anzeigen, dass ein anderer Roboter tot ist
				InformationWindow.showWindow(WatcherManager.this.presenter.getPage(), Page.props.informationWindow_dead_title(), ""
						+ Page.props.informationWindow_dead_other_text_part1() + robot.getPlayer().getUser().getAccountData().getNickname()
						+ Page.props.informationWindow_dead_other_text_part2(), "ui/robot_Lost.png");

				WatcherManager.this.presenter.getOthersStateRecords().get(robot.getPlayer().getUser().getId()).setDead();
				WatcherManager.this.presenter.getOthersReadyRecords().get(robot.getPlayer().getUser().getId()).setDead();
		...
		private List<List<Animate>> startMachines(final Timer timer) {
			 ...
			 */
			if (WatcherManager.this.roundOption.isAlwaysConveyorBeltsOn()) {

				/*
				 * Förderbänder mit Reichweite 2 werden um 1 bewegt.
				 */
				if (!WatcherManager.this.playingBoard.getConveyorBeltFieldRangeTwoList().isEmpty()) {
				 ...
				 */
				if (!WatcherManager.this.playingBoard.getConveyorBeltFieldRangeOneList().isEmpty()
						|| !WatcherManager.this.playingBoard.getConveyorBeltFieldRangeTwoList().isEmpty()) {
			 ...
			 * Pressen
			 */
			if (WatcherManager.this.roundOption.isAlwaysCompactorsOn()) {
			 ...
			 * Pusher
			 */
			if (WatcherManager.this.roundOption.isAlwaysPushersOn()) {
			 ...
			 * Zahnräder
			 */
			if (WatcherManager.this.roundOption.isAlwaysGearsOn()) {
			 ...
			 */
			if (WatcherManager.this.roundOption.isLasersOn()) {
	...
	/** der WatcherManager muss die Spielregeln kennen */
	...
	/** Presenter */
	private RoundWatcherPagePresenter presenter;
	 ...
	 * Default-Konstruktor
	 */
	public WatcherManager(RoundWatcherPagePresenter presenter) {
	...
	public boolean isGameFinished() {
		...
		if (numberOfRobots == 1) {
			...
			// WinnerWindow anzeigen
			this.presenter.getPage().addChild(
					new WinnerWindowPresenter(presenter.getRoundInfo().getRoundId(), this.presenter,
							winner.getPreviewImagePath(), winner.getPlayer().getUser().getAccountData().getNickname(), Page.props
